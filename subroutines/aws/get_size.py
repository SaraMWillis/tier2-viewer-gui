...
def du(bucket,prefix):
    ...
    if prefix == None:
        ...
        p = subprocess.Popen(['aws s3api list-objects --bucket %s --query "[length(Contents[])]"'%(bucket)],stdout=subprocess.PIPE,stdin=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
    else:
        p = subprocess.Popen(['aws s3api list-objects --bucket %s --prefix=%s --query "[length(Contents[])]"'%(bucket, prefix)],stdout=subprocess.PIPE,stdin=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
