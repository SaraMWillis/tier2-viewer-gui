...
from subroutines.formatting.bytes_to_readable import human_readable
...
'''    
def aws_ls(bucket, prefix,is_directory,get_file): 
    print("AWS LS PREFIX: %s"%prefix)
    ...
    print(path)
    print("aws s3 ls '%s/'"%path)
    if is_directory == True and prefix not in ["",None]:
        path += '/'
    print(path)
    path = '"' + path + '"'
    ...
    for obj in out:
        ...
        if len(reformatted) == 1 and reformatted[0][:4] == "PRE " and get_file == False:
            ...
            true_path = os.path.join(prefix, name) #+ '"'
        ...
        # If two objects are in our results, the regex pattern was matched. This is a file, so we'll get information from it. 
            ...
            if is_directory == False and prefix != "":
                ...
                if name == key_comparison:
                    max_name_length = len(name)
                    objects[name] = {"size" : human_readable(size), "directory": directory, "true path": true_path, "storage class": "-","ArchiveStatus": "-", "RestoreStatus":"-"}
                    break
            else:
                date, time, size =[i for i in re.search(regex_pattern,obj)[0].split(" ") if i != ""]
                directory = False
...
'''        
def aws_list_objects(bucket_limit,bucket_contents,bucket,searchable_prefix):
    if searchable_prefix == None:
        p = subprocess.Popen(["aws s3api list-objects --bucket %s --output json --max-items=%s"%(bucket,bucket_limit)],stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
    else:
        p = subprocess.Popen(["aws s3api list-objects --bucket %s --output json --prefix=%s --max-items=%s"%(bucket,searchable_prefix,bucket_limit)],stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
    ...
    if err == "" and out == "":
        print("Oops! An error occured. Verify your input to ensure you're entering the correct bucket/prefix.")
    out = json.loads(out)
    print(out)
    return out